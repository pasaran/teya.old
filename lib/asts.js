...
asts.import.options = {
    props: 'filename namespace'
};

asts.import.w_def = function() {
    var a_module = teya.parse( this.resolved_filename );
...
asts.def_func.get_full_name = function() {
    return this.name + '( ' + this.args.get_signature() + ' )';
};
...
asts.def_func.is_compatible_signature = function( inline_func_args ) {
    ...
    while ( i < l1 ) {
        ...
        if ( !( arg && types.is_castable( inline_func_arg.get_type(), arg.get_type() ) ) ) {
            return false;
        }
...
asts.def_args.w_action = function() {
    //  Validation.
...
asts.inline_func.w_prepare = function() {
    ...
    this.args.for_each( function( arg, i ) {
        ...
        arg.value.cast( def_arg.get_type() );
    } );
};
