...
asts.import.options = {
    props: 'filename namespace templates'
};

asts.import._init = function() {
    this.templates = this.ast( 'module_templates' );
};

asts.import.w_def = function() {
    var a_module = teya.parse( this.filename );
    ...
    for ( var i = 0, l = templates.length; i < l; i++ ) {
        ...
        var proxy = this.child( 'def_imported_template', {
            namespace: namespace,
            def: template,
            import: this
        } );
        this.templates.add( proxy );

        scope.add_template( proxy );
    }
};
