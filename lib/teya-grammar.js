...
rules.module = function( parser ) {
    ...
    var vars = ast.vars = parser.ast( 'module_vars' );
    var defs = ast.defs = parser.ast( 'module_defs' );
...
rules.expr = function( parser, params ) {
    ...
    if ( expr.id === 'inline_string' && parser.is_token( ':' ) ) {
        ...
        ast = parser.match( 'inline_items', item );
...
rules.template = function( parser ) {
    ...
    ast.content = parser.match( 'block', { args: true } );
...
rules.xml = function( parser ) {
    ...
    parser.token( '<' );
    ast.name = parser.match( 'string_content', { no_esc: true, delim: 'XMLNAME_END' } );
    ...
    if ( parser.is_token( '/>' ) ) {
        ...
        parser.token( '>' );
        ast.content = parser.match( 'block' );
    }
...
rules.inline_primary = function( parser ) {
    ...
    if ( parser.is_token( 'NUMBER' ) ) {
        ...
        switch ( id ) {
            case 'true':
                expr = parser.match( 'true' );
                break;

            case 'false':
                expr = parser.match( 'false' );
                break;
