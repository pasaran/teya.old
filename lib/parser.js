...
teya.Parser.prototype.first_pass = function() {
    ...
    while ( !this.is_eof() ) {
        ...
        if ( this.is_token( '.' ) ) {
            //  def_template foo.bar
            while ( this.is_token( '.' ) ) {
                this.move_x( 1 );
                id += '.' + this.token( 'id' );
            }
            templates_names[ id ] = true;
            ...
            if ( this.is_token( '=' ) ) {
                //  def_var: foo = ...
                this.move_skip( 1, 'spaces' );

                if ( this.is_token( 'require' ) ) {
                    //  require: foo = require "..."
                    //
                    //  'require'.length === 7.
                    this.move_skip( 7, 'spaces' );
                    ...
                    modules[ id ] = teya.parse( filename );

                    //  FIXME: Не нужно ли тут проверить, что дальше EOL?
