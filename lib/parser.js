...
teya.Parser.prototype.first_pass = function() {
    ...
    while ( !this.is_eof() ) {
        ...
        if ( id === 'import' ) {
            ...
            var filename = this.match( 'inline_string', { no_expr: true, no_esc: true } ).as_string();
            ...
            if ( /\.teya$/.test( filename ) ) {
                ...
                var module_templates = a_module.templates.items;
                for ( var i = 0, l = module_templates.length; i < l; i++ ) {
                    var template = module_templates[ i ];
                    var template_name = ( namespace ) ? namespace + '.' + template.name : template.name;
...
teya.Parser.prototype.ast = function( id, params, pos ) {
    pos = pos || this.get_pos();

    return this.factory.ast( id, pos, params );
};
