...
Parser.prototype._init = function( grammar, ctors ) {
...
Parser.prototype.on_load = no.nop;
Parser.prototype.on_line_start = no.nop;
Parser.prototype.on_line_end = no.nop;
...
Parser.prototype.error = function( error ) {
    Terror.throw( error, this.get_pos() );
};
...
Parser.prototype.is_any_token = function() {
    ...
    this.error( {
        id: 'TOKENS_EXPECTED',
        tokens: arguments,
        msg: 'Expected: ' + [].join.call( arguments, ', ' )
    } );
};
...
Parser.prototype.ast = function( id, params ) {
    ...
    if ( !ctor ) {
        //  FIXME: Убрать это потом.
        console.log( 'NO AST', id );
    }

    return new ctor( this.get_pos(), params );
};
