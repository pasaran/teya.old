...
Parser.prototype.add_template = function( template ) {
    ...
    this.template_names[ name ] = true;
};

Parser.prototype.is_template = function( name, is_full_name ) {
    var template_names = this.template_names;

    if ( is_full_name ) {
        return template_names[ name ];
    }

    var template_calls = this.template_calls;
    for ( var i = 0, l = template_calls.length; i < l; i++ ) {
        var template_name = template_calls.slice( i ).join( '.' );
        if ( template_names[ template_name ] ) {
            return true;
        }
    }
};
