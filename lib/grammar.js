...
rules.module = function( parser ) {
    ...
    while ( !parser.is_eof() ) {
        ...
        //
            ...
            def = parser.match( 'def_var', ast );
        } else {
            def = parser.match( 'def_template', ast );
        }
...
rules.def_template = function( parser, parent ) {
    var ast = parser.ast( 'def_template', ast );

    ast.name = parser.token( 'ID' );
    if ( parser.is_token( '(' ) ) {
        ast.args = parser.match( 'template_args', ast );
    }
    ast.body = parser.match( 'body', ast );

    return ast;
};

rules.def_var = function( parser, parent ) {
    var ast = parser.ast( 'def_var', parent );
...
rules.def_func = function( parser, parent ) {
    var ast = parser.ast( 'def_func', parent );

    parser.token( 'func' );
    ast.name = parser.token( 'ID' );
    ast.args = parser.match( 'func_args', ast );
    ast.body = parser.match( 'body', ast );

    return ast;
};

rules.func_args = function( parser, parent ) {
    var ast = parser.ast( 'func_args', parent );

    parser.token( '(' );
    if ( !parser.is_token( ')' ) ) {
        ast.add( parser.match( 'func_arg', ast ) );
...
rules.func_arg = function( parser, parent ) {
    var ast = parser.ast( 'func_arg', parent );
...
rules.expr = function( parser, parent ) {
    ...
    */
    ...
    switch ( char ) {
        ...
        case '"':
        case "'":
            var key = parser.match( 'inline_expr', parent );
            if ( !parser.is_token( ':' ) ) {
                return key;
            }

            var pair = parser.ast( 'pair', parent );
            //  FIXME: Так плохо, надо как-то правильнее.
            key.parent = pair;

            pair.key = key;
            parser.token( ':' );
            pair.value = parser.match( 'expr', pair );
...
rules.block = function( parser, parent ) {
    ...
    while ( !parser.is_eof() && !parser.is_dedent() ) {
        ...
        if ( parser.is_all_tokens( 'ID', '=' ) ) {
            ast.add( parser.match( 'def_var', ast ) );
...
rules.inline_primary = function( parser, parent ) {
    ...
    if ( parser.is_token( 'NUMBER' ) ) {
        ast = parser.match( 'inline_number', parent );

    } else if ( parser.is_token( '"' ) || parser.is_token( "'" ) ) {
        ast = parser.match( 'inline_string', parent );

    } else {
        if ( parser.is_token( '(' ) ) {
            ast = parser.match( 'inline_subexpr', parent );

        } else if ( parser.is_token( '.' ) || parser.is_token( '/' ) ) {
            ast = parser.match( 'jpath', parent );

        } else if ( parser.is_all_tokens( 'ID', '(' ) ) {
            ast = parser.match( 'func', parent );

        } else if ( parser.is_token( 'ID' ) ) {
            ast = parser.match( 'var', parent );
