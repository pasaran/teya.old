...
function r_def_template( parser ) {
    ...
    if ( parser.is_token( '(' ) ) {
        ...
        //  FIXME: Хорошо бы, чтобы пустой объект приходил из r_def_args всегда.
        ast.args = parser.ast( 'def_args' );
    }
...
function r_import( parser ) {
    ...
    ast.resolved_filename = path_.resolve( dirname, ast.filename );
    ...
    if ( /\.js$/.test( ast.filename ) ) {
        ast.is_js = true;

        ast.externals = r_externals( parser );
    }

    return ast;
}

function r_externals( parser ) {
    var ast = parser.ast( 'externals' );
    ...
    while ( !parser.is_eof() && !parser.is_dedent() ) {
        ...
        ast.add( r_def_external_func( parser ) );
    }
...
function r_def_external_func( parser ) {
    ...
    //  FIXME: Передвинуть это наверх. Для этого нужно, чтобы
    //  _init мог принимать undefined. А для этого нужно, чтобы
    //  был отдельный ast для сигнатуры, _init которой принимает массив строк-типов.
    //
    return parser.ast( 'def_external_func', {
        name: name,
        args: args,
        type: type
    }, pos );
...
//
function r_def_args( parser, types_only ) {
    var ast = parser.ast( 'def_args' );

    parser.token( '(' );

    if ( !parser.is_token( ')' ) ) {
        ast.add( r_def_arg( parser, types_only ) );
        while ( parser.is_token( ',' ) ) {
            parser.move_skip( 1 );
            ast.add( r_def_arg( parser, types_only ) );
        }
...
//
function r_def_arg( parser, types_only ) {
...
function r_attr( parser ) {
    ...
    //  FIXME: А почему тут имя всегда статичное?
    //  Как насчет:
    //
    //      @data-{ .name } = .value
    //
    ...
    if ( parser.is_token( '+=' ) ) {
        ...
        //  FIXME: Неплохо бы проверить, что там таки '+', а не какая-нибудь лажа.
        parser.move_skip( 1 );
        ast.op = '+';
    }
    ast.value = r_expr( parser );
...
function r_inline_primary( parser ) {
    ...
    if ( parser.is_token( 'number' ) ) {
        ...
        expr = r_inline_object( parser );

    } else if ( parser.is_token( '[' ) ) {
        expr = r_inline_array( parser );
...
function r_inline_object( parser ) {
    var ast = parser.ast( 'inline_object' );
...
function r_inline_array( parser ) {
    var ast = parser.ast( 'inline_array' );
...
function match_type( parser ) {
    ...
    return ( is_rest ) ? TYPE.REST( id ) : id;
}
