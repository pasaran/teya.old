...
function r_def_template( parser ) {
    ...
    parser.template_calls.push( name );
    ast.body = r_block( parser );
    parser.template_calls.pop();
...
function r_expr( parser, params ) {
    ...
    if ( id ) {
        ...
        var template_names = parser.template_names;
        //
        if ( parser.is_token( '.' ) ) {
            ...
            is_template = !!template_names[ name ];

        } else {
            var template_calls = parser.template_calls;
            var postfix = '.' + id;

            for ( var i = 0, l = template_calls.length; i < l; i++ ) {
                name = template_calls.slice( i ).join( '.' ) + postfix;
            ...
            if ( !is_template && template_names[ id ] ) {
                is_template = true;
                name = id;
            }
