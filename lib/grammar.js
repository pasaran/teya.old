...
tokens.esc = function t_esc( parser ) {
    ...
    //  "'\nt
    return ( c === 34 || c === 39 || c === 92 || c === 110 || c === 116 );
}
...
tokens[ '|' ] = function( parser ) {
    ...
    //  FIXME: Посмотреть, не будет ли тут деоптимизации в конце строки,
    //  когда `line.charCodeAt()` вернет `NaN`.
    //
    return ( line.charCodeAt( x ) === 124 && line.charCodeAt( x + 1 ) !== 124 );
};
...
tokens[ '=' ] = function( parser ) {
    ...
    return ( line.charCodeAt( x ) === 61 && line.charCodeAt( x + 1 ) !== 61 );
};
...
function r_module( parser ) {
    ...
    while ( !parser.is_eof() ) {
        ...
        if ( parser.is_token( 'func' ) ) {
            def = r_def_func( parser );

            defs.add( def );

        } else if ( parser.is_all_tokens( 'id', '=' ) ) {
            def = r_def_var( parser);

            vars.add( def );

        } else {
            def = r_def_template( parser );
...
function r_block( parser, params ) {
...
function r_def_template( parser ) {
    ...
    */
    ...
    //  FIXME: Это будет делаться в первом проходе.
...
rules.def_args = function( parser ) {
    var ast = parser.ast( 'def_args' );

    parser.token( '(' );
    if ( !parser.is_token( ')' ) ) {
        ast.add( parser.match( 'def_arg' ) );
...
//  def_arg := TYPE? ID ( '=' inline_expr )?

rules.def_arg = function( parser ) {
...
function r_expr = function( parser, params ) {
    switch ( parser.next( 3 ) ) {
        case '...':
...
function r_for = function( parser ) {
...
function r_template = function( parser ) {
    ...
    */
    ...
    //  FIXME: Тут нужен инлайн-контент и пустой контент.
    ast.content = r_block( parser );
    //  ast.content = r_block( parser, { args: true } );
...
function r_xml( parser ) {
    ...
    if ( parser.is_token( '/>' ) ) {
        ...
        parser.move_skip( 2 );
        //  Создаем пустой блок.
        //  FIXME: Таки нужно какое-то общее правило для `body`.
        ast.content = parser.ast( 'block' );

    } else {
        parser.token( '>' );
        //  FIXME: Тут может быть пустой блок.
        ast.content = r_block( parser );
    }
...
function r_inline_string( parser, params ) {
    ...
    if ( parser.is_token( quote ) ) {
        ...
        //  FIXME: А почему string_content ломается на пустой строке?
        parser.move_skip( 1 );
...
function r_jpath( parser, params ) {
    ...
    //  FIXME: Тут неправильно обрабатывается / и .
    //
