...
function r_def_var( parser ) {
    ...
    if ( parser.is_token( 'require' ) ) {
        ast.value = r_require( parser );
...
//  require := 'require' inline_string
//
function r_require( parser ) {
    var ast = parser.ast( 'require' );

    //  Здесь всегда require.
    //  'require'.length === 7.
    parser.move_skip( 7 );
    ast.filename = r_inline_string( parser, { no_expr: true, no_esc: true } );
