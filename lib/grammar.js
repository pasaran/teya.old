...
rules.inline_primary = function( parser, parent ) {
    ...
    if ( parser.is_token( '(' ) ) {
        ...
        expr = parser.match( 'inline_function', parent );

    } else if ( parser.is_token( 'ID' ) ) {
        expr = parser.match( 'inline_var', parent );
    ...
    if ( parser.is_token( '[' ) || parser.is_token( '.' ) ) {
        ...
        filter.expr = expr;
        filter.jpath = parser.match( 'jpath', parent, { inContext: true } );
...
rules.inline_var = function( parser, parent ) {
    var ast = new AST.inline_var( parent );
...
rules.inline_function = function( parser, parent ) {
    var ast = new AST.inline_function( parent );
