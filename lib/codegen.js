    ...
    for ( var mode in modes ) {
        ( function( mode, method ) {
            ...
            base.prototype[ method_key ] = function() {
                //  console.log( 'TEMPLATE', this.id, mode );
                return method( this ) || '';
            };

            asts.items[ method_key ] = function() {
                //  console.log( 'TEMPLATE.ITEMS', this.id, mode );
                var r = method( this );
                if ( r != null ) {
                    return r;
                }
...
function compile_templates( lang ) {
    var filename = path_.join( __dirname, '/../templates/' + lang + '.tmpl' );
    ...
    for ( var mode in templates ) {
        ...
        for ( var i = 0, l = mode_templates.length; i < l; i++ ) {
            ...
            var condition = '';
            if ( template.id !== '*' ) {
                //  FIXME: Если тут просто сравнивать id с template.id,
                //  то пропадает возможность "наследовать" шаблоны.
                //  Т.е., например, написать шаблон для inline_binop.
                //
                //  Но, возможно, это сильно дороже. Так что нужно потом померять.
                //
                condition += 'ast.is("' + template.id + '")';
            }
...
function macro( ast, method ) {
    ...
    if ( typeof ast[ method ] !== 'function' ) {
        console.log( 'No method "%s()" in "%s"', method, ast.id );
    }
    return ast[ method ]();
}
